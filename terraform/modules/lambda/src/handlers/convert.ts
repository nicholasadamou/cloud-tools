...
export const handler = async (
  ...
  try {
    ...
    const { jobId, inputFormat, outputFormat, fileName, s3Key, quality } = JSON.parse(event.body);
    ...
    if (!jobId || !inputFormat || !outputFormat || !fileName || !s3Key) {
      return {
        ...
        body: JSON.stringify({
          ...
          required: ['jobId', 'inputFormat', 'outputFormat', 'fileName', 's3Key'],
        }),
      };
    }
    ...
    try {
      ...
      if (!processor) {
        throw new Error(`No processor found for operation: convert, format: ${outputFormat}`);
      }
