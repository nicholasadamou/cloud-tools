...
export const handler = async (event: SQSEvent, context: Context): Promise<void> => {
  ...
  const promises = event.Records.map(async (record: SQSRecord) => {
    try {
      ...
      const outputPrefix = processingMessage.operation === 'compress' ? 'compressed' : 'processed';
      ...
      dependencies.logger.info(`Processing completed for job ${processingMessage.jobId}`, {
        operation: processingMessage.operation,
        targetFormat: processingMessage.targetFormat,
      ...
      try {
        ...
        dependencies.logger.error('Could not update job status due to parse error', parseError);
      }
  ...
  try {
    ...
    dependencies.logger.info(`Successfully processed all ${event.Records.length} messages`);
  } catch (error) {
