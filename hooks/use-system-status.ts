...
export function useSystemStatus() {
  ...
  const checkSystemStatus = async () => {
    try {
      ...
      const isHealthy = healthCheck.status === 'fulfilled' && healthCheck.value.ok
      ...
      if (localStackCheck.status === 'fulfilled' && localStackCheck.value.ok) {
        try {
          ...
          const keyServicesRunning = [s3Status, dynamodbStatus, sqsStatus]
            .filter(serviceStatus => serviceStatus === 'running' || serviceStatus === 'available')
