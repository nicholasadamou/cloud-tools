...
vi.mock("@/root-lib/aws-config", async () => {
  ...
  return {
    ...
    getSQSQueueUrl: vi
      .fn()
      .mockResolvedValue("http://localhost:4566/000000000000/test-queue"),
  };
});
...
describe("/api/process", () => {
  beforeEach(async () => {
    ...
    const { getSQSQueueUrl } = await import("@/root-lib/aws-config");
    vi.mocked(getSQSQueueUrl).mockResolvedValue(
  ...
  describe("GET", () => {
    it("should return queue status", async () => {
      const request = createMockGetRequest(
        "http://localhost:3000/api/process?action=queue-status",
      );
    ...
    it("should return error for invalid action", async () => {
      const request = createMockGetRequest(
        "http://localhost:3000/api/process?action=invalid",
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it("should handle queue status retrieval error", async () => {
      ...
      const { getSQSQueueUrl } = await import("@/root-lib/aws-config");
      vi.mocked(getSQSQueueUrl).mockRejectedValueOnce(
        new Error("SQS service error"),
      );
  ...
  describe("DELETE", () => {
    it("should cancel a processing job successfully", async () => {
      ...
      const request = createMockDeleteRequest(
        "http://localhost:3000/api/process?jobId=test-job-123",
      );
