...
import { RefreshCcw, Minimize2, Image, FileAudio, FileVideo, BookOpen, FileImage, FileText } from 'lucide-react'
						...
						<Card className={cn(
							...
							<CardContent className="flex flex-col items-center justify-center p-4 h-full">
								<tool.icon className={cn(
									...
									pathname === tool.href ? 'text-primary' : 'text-muted-foreground group-hover:text-primary'
								)} />
								<span className={cn(
				...
				>
					<Card className="hover:bg-accent/50 overflow-hidden">
						<CardContent className="flex flex-col items-center justify-center p-4 h-full">
							...
							<span className="text-sm font-medium text-center text-muted-foreground">{otherCategory}</span>
						</CardContent>
					</Card>
				</Link>
			</div>
