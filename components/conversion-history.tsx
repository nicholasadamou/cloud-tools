...
import { Download, RefreshCw, Clock, CheckCircle, XCircle, Trash2, Image as ImageIcon } from 'lucide-react'
          ...
          try {
            ...
            if (response.ok) {
              ...
              const updatedRecord = {
                ...
                url: jobData.data?.status === 'completed' && jobData.data?.downloadUrl 
                  ? jobData.data.downloadUrl 
                  : record.url,
                // Add compression data if available
                originalFileSize: jobData.data?.originalFileSize || record.originalFileSize,
                processedFileSize: jobData.data?.processedFileSize || record.processedFileSize,
                compressionSavings: jobData.data?.compressionSavings || record.compressionSavings
              }
              
              // Check if anything changed
              if (updatedRecord.status !== record.status || updatedRecord.url !== record.url) {
                hasUpdates = true
              }
            ...
            console.error(`Failed to update job status for ${record.jobId}:`, error)
          }
        }
        return record
      })
      ...
      <Card className="w-full h-[600px]">
        <div className="flex items-center justify-between p-4 border-b">
          <div className="flex items-center space-x-2">
            ...
            {history.some(record => record.status === 'processing' || record.status === 'pending') && (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {record.operation === 'compress' && record.compressionSavings && record.originalFileSize && record.processedFileSize && (
                          <div className="text-xs text-green-600 mt-1 flex items-center space-x-2">
                            <span>✓ {record.compressionSavings.toFixed(1)}% saved</span>
                            <span>•</span>
                            <span>{((record.originalFileSize - record.processedFileSize) / (1024 * 1024)).toFixed(1)}MB reduced</span>
                          </div>
                        )}
                        <p className="text-sm text-muted-foreground">{new Date(record.date).toLocaleString()}</p>
                        <div className="flex items-center space-x-4 mt-1">
                          <p className="text-xs text-muted-foreground">Job ID: {record.jobId}</p>
                          <div className="flex items-center space-x-1">
                            <span className={`inline-block w-2 h-2 rounded-full ${
                              record.status === 'processing' || record.status === 'pending' ? 'bg-blue-500 animate-pulse' :
                              record.status === 'completed' ? 'bg-green-500' :
                              record.status === 'failed' ? 'bg-red-500' : 'bg-gray-500'
                            }`} />
